...
const data = ref<TektonTaskRun[]>([])
const selectedRows = ref<TektonTaskRun[]>([])
...
const fetchTektonTaskRuns = async () => {
  try {
    const response = await getTektonTaskRuns() as ApiResponse<TektonTaskRun[]>
...
const handleSelectionChange = (rows: TektonTaskRun[]) => {
  selectedRows.value = rows
}

const handleDelete = async (taskRunName: string) => {
  try {
    await ElMessageBox.confirm(
      `此操作将永久删除 Tekton TaskRun ${taskRunName}，是否继续？`,
      '提示',
      {
    ...
    const response = await deleteTektonTaskRun(taskRunName) as ApiResponse
    if (!response.success) {
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `此操作将永久删除选中的 ${names.length} 条 TaskRun，是否继续？`,
      '提示',
      {
        confirmButtonText: '确 认',
        cancelButtonText: '取 消',
    ...
    const response = await deleteTektonTaskRuns(names) as ApiResponse
    if (!response.success) {
