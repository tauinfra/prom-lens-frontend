...
import {
  ...
  updatePersistentVolumeClaim, createPersistentVolumeClaim, deletePersistentVolumeClaim, batchDeletePersistentVolumeClaims
} from '@/api/kubernetes'
...
interface PersistentVolumeClaim {
  ...
  accessModes: string[];
  status: string;
  storageClass?: string | null; // 对应 *string，可能为 null
  volume: string;
  createdAt: string;
}
...
const name = ref() // PersistentVolumeClaim name

const data = ref<PersistentVolumeClaim[]>([]);
const selectedPersistentVolumeClaimNames = ref<string[]>([])
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: PersistentVolumeClaim) =>
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchPersistentVolumeClaims(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
 ...
 */
const fetchPersistentVolumeClaims = async (clusterId: number, ns: string) => {
  try {
    const response = await getPersistentVolumeClaims(clusterId, ns) as ApiResponse<[]>
...
/**
 * 获取 PersistentVolumeClaim YAML 配置
 */
const fetchPersistentVolumeClaim = async (clusterId: number, ns: any, name: string) => {
  try {
    const response = await getPersistentVolumeClaim(clusterId, ns, name) as ApiResponse<[]>
...
const refreshClick = () => {
  ...
  fetchPersistentVolumeClaims(clusterId.value, namespace.value)
}

/**
 * 新增 PersistentVolumeClaim（YAML）
 */
...
const handleYamlClick = (row: any) => {
  ...
  fetchPersistentVolumeClaim(clusterId.value, namespace.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createPersistentVolumeClaim(clusterId.value, namespace.value, jsonData) as ApiResponse
      : await updatePersistentVolumeClaim(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse;
    ...
    message(`PersistentVolumeClaim '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleDelete = async (name: string) => {
  try {
    ...
    const response = await deletePersistentVolumeClaim(clusterId.value, namespace.value, name) as ApiResponse
    if (!response.success) {
...
const handleSelectionChange = (selection: PersistentVolumeClaim[]) => {
  selectedPersistentVolumeClaimNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedPersistentVolumeClaimNames.value.length === 0) {
    message('请先选择要删除的 PersistentVolumeClaim', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedPersistentVolumeClaimNames.value.length} 个 PersistentVolumeClaim，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeletePersistentVolumeClaims(clusterId.value!, namespace.value!, { names: selectedPersistentVolumeClaimNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`PersistentVolumeClaim 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchPersistentVolumeClaims(clusterId.value!, namespace.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedPersistentVolumeClaimNames.length > 0"
        type="danger"
        plain
    ...
    <el-table :data="pagedData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed prop="volume" label="关联存储卷"></el-table-column>
      <el-table-column fixed prop="capacity" label="容量"></el-table-column>
      <el-table-column fixed prop="accessModes" label="访问模式"></el-table-column>
      <el-table-column fixed prop="storageClass" label="存储类型"></el-table-column>
