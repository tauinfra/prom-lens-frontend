...
import {
  ...
  getPersistentVolume,
  updatePersistentVolume, createPersistentVolume, deletePersistentVolume, batchDeletePersistentVolumes
} from '@/api/kubernetes'
...
interface PersistentVolume {
  name: string;
  capacity: string;
  accessModes: string[];           // 直接用 string[], 前端展示更方便
  reclaimPolicy: string;           // 如 "Retain" | "Recycle" | "Delete"
  status: string;                  // 如 "Bound" | "Available" | "Released"
  claim: string;                   // 如 "default/my-persistentvolumeclaim"
  storageClass: string;            // 如 "standard"
  reason: string;                  // 状态原因
  createdAt: string;               // 时间字符串
}
...
const name = ref() // PersistentVolume name

const data = ref<PersistentVolume[]>([]);
const selectedPersistentVolumeNames = ref<string[]>([])
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: PersistentVolume) =>
 ...
 */
const fetchPersistentVolumes = async (clusterId: number) => {
  try {
    const response = await getPersistentVolumes(clusterId) as ApiResponse<PersistentVolume[]>
    if (!response.success) {
      showApiError(response)
      return
    }
...
/**
 * 获取 PersistentVolume YAML 配置
 */
const fetchPersistentVolume = async (clusterId: number, name: string) => {
  try {
    const response = await getPersistentVolume(clusterId, name) as ApiResponse<[]>
    if (!response.success) {
      showApiError(response)
      return
    }
...
const refreshClick = () => {
  ...
  fetchPersistentVolumes(clusterId.value)
}

/**
 * 新增 PersistentVolume（YAML）
 */
...
const handleYamlClick = (row: any) => {
  ...
  fetchPersistentVolume(clusterId.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createPersistentVolume(clusterId.value, jsonData) as ApiResponse
      : await updatePersistentVolume(clusterId.value, name.value, jsonData) as ApiResponse;
    ...
    message(`PersistentVolume '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleDelete = async (name: string) => {
  try {
    ...
    const response = await deletePersistentVolume(clusterId.value, name) as ApiResponse
    if (!response.success) {
      showApiError(`资源 '${name}' 删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchPersistentVolumes(clusterId.value)
...
const handleSelectionChange = (selection: PersistentVolume[]) => {
  selectedPersistentVolumeNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedPersistentVolumeNames.value.length === 0) {
    message('请先选择要删除的 PersistentVolume', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedPersistentVolumeNames.value.length} 个 PersistentVolume，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeletePersistentVolumes(clusterId.value!, { names: selectedPersistentVolumeNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`PersistentVolume 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchPersistentVolumes(clusterId.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedPersistentVolumeNames.length > 0"
        type="danger"
        plain
    ...
    <el-table :data="pagedData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="capacity" label="容量" width="120"></el-table-column>
      <el-table-column prop="accessModes" label="访问模式"></el-table-column>
      <el-table-column prop="reclaimPolicy" label="回收策略" width="120"></el-table-column>
      <el-table-column prop="status" label="状态" width="120"></el-table-column>
      <el-table-column prop="claim" label="绑定存储声明" width="270"></el-table-column>
      <el-table-column prop="storageClass" label="存储类型"></el-table-column>
