...
import {
  ...
  getServices,
  getService,
  updateService, createService, deleteService, batchDeleteServices
} from '@/api/kubernetes'
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: Service) =>
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchServices(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
 ...
 * 获取 Service 列表
 */
const fetchServices = async (clusterId: number, ns: string) => {
  try {
    const response = await getServices(clusterId, ns) as ApiResponse<[]>
 ...
 * 获取 Service YAML 配置
 */
const fetchService = async (clusterId: number, ns: any, name: string) => {
  try {
    const response = await getService(clusterId, ns, name) as ApiResponse<[]>
...
const refreshClick = () => {
  ...
  fetchServices(clusterId.value, namespace.value)
}
...
const handleYamlClick = (row: any) => {
  ...
  fetchService(clusterId.value, namespace.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createService(clusterId.value, namespace.value, jsonData) as ApiResponse
      : await updateService(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse;
    ...
    message(`Service '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleDelete = async (name: string) => {
  try {
    ...
    const response = await deleteService(clusterId.value, namespace.value, name) as ApiResponse
    if (!response.success) {
...
const handleSelectionChange = (selection: Service[]) => {
  selectedServiceNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedServiceNames.value.length === 0) {
    message('请先选择要删除的 Service', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedServiceNames.value.length} 个 Service，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeleteServices(clusterId.value!, namespace.value!, { names: selectedServiceNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`Service 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchServices(clusterId.value!, namespace.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedServiceNames.length > 0"
        type="danger"
        plain
