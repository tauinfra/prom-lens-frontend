...
import {
  ...
  getConfigMaps,
  getConfigMap,
  updateConfigMap, createConfigMap, deleteConfigMap, batchDeleteConfigMaps
} from '@/api/kubernetes'
...
const name = ref() // ConfigMap name

const data = ref<ConfigMap[]>([]);
const selectedConfigMapNames = ref<string[]>([])
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: ConfigMap) =>
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchConfigMaps(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
 ...
 * 获取 ConfigMap 列表
 */
const fetchConfigMaps = async (clusterId: number, ns: string) => {
  try {
    const response = await getConfigMaps(clusterId, ns) as ApiResponse<[]>
...
/**
 * 获取 ConfigMap YAML 配置
 */
const fetchConfigMap = async (clusterId: number, ns: any, name: string) => {
  try {
    const response = await getConfigMap(clusterId, ns, name) as ApiResponse<[]>
...
const refreshClick = () => {
  ...
  fetchConfigMaps(clusterId.value, namespace.value)
}
...
const handleYamlClick = (row: any) => {
  ...
  fetchConfigMap(clusterId.value, namespace.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createConfigMap(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse
      : await updateConfigMap(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse;
    ...
    message(`ConfigMap '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleDelete = async (name: string) => {
  try {
    ...
    const response = await deleteConfigMap(clusterId.value, namespace.value, name) as ApiResponse
    if (!response.success) {
...
const handleSelectionChange = (selection: ConfigMap[]) => {
  selectedConfigMapNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedConfigMapNames.value.length === 0) {
    message('请先选择要删除的 ConfigMap', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedConfigMapNames.value.length} 个 ConfigMap，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeleteConfigMaps(clusterId.value!, namespace.value!, { names: selectedConfigMapNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`ConfigMap 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchConfigMaps(clusterId.value!, namespace.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedConfigMapNames.length > 0"
        type="danger"
        plain
