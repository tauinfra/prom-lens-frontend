...
import {
  ...
  getKubeRoleBindings,
  getKubeRoleBinding,
  updateKubeRoleBinding,
  createKubeRoleBinding,
  deleteKubeRoleBinding,
  batchDeleteKubeRoleBindings
} from '@/api/kubernetes'
...
const selectedRoleBindingNames = ref<string[]>([])
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchRoleBindings(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
...
const fetchRoleBindings = async (cid: number, ns: string) => {
  try {
    const response = await getKubeRoleBindings(cid, ns) as ApiResponse<RoleBinding[]>
...
const fetchRoleBinding = async (cid: number, ns: string, bindingName: string) => {
  try {
    const response = await getKubeRoleBinding(cid, ns, bindingName) as ApiResponse<unknown>
...
const refreshClick = () => {
  ...
  fetchRoleBindings(clusterId.value!, namespace.value!)
}
...
const handleYamlClick = (row: RoleBinding) => {
  name.value = row.name
  dialogStatus.value = 'update'
  fetchRoleBinding(clusterId.value!, namespace.value!, row.name)
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createKubeRoleBinding(clusterId.value!, namespace.value!, name.value, jsonData) as ApiResponse
      : await updateKubeRoleBinding(clusterId.value!, namespace.value!, name.value, jsonData) as ApiResponse

    if (!response.success) {
      showApiError(`RoleBinding '${name.value}' ${operation === 'create' ? '添加' : '更新'}失败. 错误信息: ${response.msg}`)
      return
    }
    dialogVisible.value = false
    message(`RoleBinding '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, { type: "success", duration: 5000 })
    await fetchRoleBindings(clusterId.value!, namespace.value!)
  } catch (error) {
    if (error !== 'cancel') {
...
const handleDelete = async (bindingName: string) => {
  try {
    ...
    const response = await deleteKubeRoleBinding(clusterId.value!, namespace.value!, bindingName) as ApiResponse
    if (!response.success) {
      showApiError(`RoleBinding '${bindingName}' 删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchRoleBindings(clusterId.value!, namespace.value!)
...
const handleSelectionChange = (selection: RoleBinding[]) => {
  selectedRoleBindingNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedRoleBindingNames.value.length === 0) {
    message('请先选择要删除的 RoleBinding', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedRoleBindingNames.value.length} 个 RoleBinding，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeleteKubeRoleBindings(clusterId.value!, namespace.value!, { names: selectedRoleBindingNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`RoleBinding 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchRoleBindings(clusterId.value!, namespace.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedRoleBindingNames.length > 0"
        type="danger"
        plain
    ...
    <el-table :data="pagedData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="roleRef" label="角色" min-width="160" show-overflow-tooltip/>
    ...
    <el-dialog title="RoleBinding YAML" v-model="dialogVisible" width="60%">
      <el-form>
