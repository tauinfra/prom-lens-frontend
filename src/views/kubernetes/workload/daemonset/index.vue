...
import {
  ...
  getDaemonSets,
  getDaemonSet,
  updateDaemonSet, createDaemonSet, restartDaemonSet, deleteDaemonSet, batchDeleteDaemonSets
} from '@/api/kubernetes'
...
const { goToDaemonSetDetail } = useDetailRoutes();
...
const name = ref() // DaemonSet name

const data = ref<DaemonSet[]>([]);
const selectedDaemonSetNames = ref<string[]>([])
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: DaemonSet) =>
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchDaemonSets(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
 ...
 * 获取 DaemonSet 列表
 */
const fetchDaemonSets = async (clusterId: number, ns: string) => {
  try {
    const response = await getDaemonSets(clusterId, ns) as ApiResponse<[]>
    data.value = response.data ?? []
    selectedDaemonSetNames.value = []
  } catch (error) {
    showApiError(error)
    return []
  }
...
/**
 * 获取 DaemonSet YAML 配置
 */
const fetchDaemonSet = async (clusterId: number, ns: any, name: string) => {
  try {
    const response = await getDaemonSet(clusterId, ns, name) as ApiResponse<[]>
...
const refreshClick = () => {
  ...
  fetchDaemonSets(clusterId.value, namespace.value)
}
...
const handleYamlClick = (row: any) => {
  ...
  fetchDaemonSet(clusterId.value, namespace.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createDaemonSet(clusterId.value, namespace.value, jsonData) as ApiResponse
      : await updateDaemonSet(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse;
    ...
    message(`DaemonSet '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleRestart = async(name: string) => {
  try {
    ...
    const response = await restartDaemonSet(clusterId.value, namespace.value, name) as ApiResponse
...
const handleDelete = async (name: string) => {
  try {
    ...
    const response = await deleteDaemonSet(clusterId.value, namespace.value, name) as ApiResponse
    if (!response.success) {
...
const handleSelectionChange = (selection: DaemonSet[]) => {
  selectedDaemonSetNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedDaemonSetNames.value.length === 0) {
    message('请先选择要删除的 DaemonSet', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedDaemonSetNames.value.length} 个 DaemonSet，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeleteDaemonSets(clusterId.value!, namespace.value!, { names: selectedDaemonSetNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`DaemonSet 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchDaemonSets(clusterId.value!, namespace.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedDaemonSetNames.length > 0"
        type="danger"
        plain
    ...
    <el-table :data="pagedData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="desiredNumberScheduled" label="期望副本数" width="140"></el-table-column>
      <el-table-column prop="currentNumberScheduled" label="当前副本数" width="140"></el-table-column>
      <el-table-column prop="numberReady" label="就绪副本数" width="140"></el-table-column>
      <el-table-column prop="updatedNumberScheduled" label="更新副本数" width="140"></el-table-column>
      <el-table-column prop="numberAvailable" label="可用副本数" width="140"></el-table-column>
      <el-table-column prop="createdAt" label="创建时间" width="170"></el-table-column>
      <el-table-column fixed="right" label="操作" width="240">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="goToDaemonSetDetail(
                    {
                    id: clusterId,
                    namespace: namespace,
                    name: scope.row.name,
