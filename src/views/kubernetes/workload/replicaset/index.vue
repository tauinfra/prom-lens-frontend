...
import {
  ...
  getReplicaSets,
  getDeployment,
  updateDeployment, createDeployment, restartDeployment, scaleDeployment, deleteReplicaSet, batchDeleteReplicaSets
} from '@/api/kubernetes'
...
const { goToPodDetail } = useDetailRoutes();
...
const name = ref() // replicaset name

const data = ref<ReplicaSet[]>([]);
const selectedReplicaSetNames = ref<string[]>([])
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: ReplicaSet) =>
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchReplicaSets(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
 ...
 * 获取 ReplicaSet 列表
 */
const fetchReplicaSets = async (clusterId: number, ns: string) => {
  try {
    const response = await getReplicaSets(clusterId, ns) as ApiResponse<[]>
...
/**
 * 获取 ReplicaSet YAML 配置
 */
const fetchReplicaSet = async (clusterId: number, ns: any, name: string) => {
  try {
 ...
 */
const refreshClick = () => {
  fetchReplicaSets(clusterId.value, namespace.value)
}
...
const handleYamlClick = (row: any) => {
  ...
  fetchReplicaSet(clusterId.value, namespace.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createDeployment(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse
    ...
    message(`ReplicaSet '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleScale = async () => {
  try {
    scaleDialogVisible.value = false // 先关闭表单
    await ElMessageBox.confirm(
...
const handleDelete = async (name: string) => {
  try {
    ...
    const response = await deleteReplicaSet(clusterId.value, namespace.value, name) as ApiResponse
    if (!response.success) {
...
const handleSelectionChange = (selection: ReplicaSet[]) => {
  selectedReplicaSetNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedReplicaSetNames.value.length === 0) {
    message('请先选择要删除的 ReplicaSet', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedReplicaSetNames.value.length} 个 ReplicaSet，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const response = await batchDeleteReplicaSets(clusterId.value!, namespace.value!, { names: selectedReplicaSetNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`ReplicaSet 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchReplicaSets(clusterId.value!, namespace.value!)
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedReplicaSetNames.length > 0"
        type="danger"
        plain
