...
import { getClusters, getNamespaces, getJobs, getJob, createJob, updateJob, deleteJob } from "@/api/kubernetes";
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    pagination.value.page = 1;
    await fetchJobsData(clusterId.value, namespace.value);
  } else {
    data.value = [];
  }
};
...
const fetchJobsData = async (cid: number, ns: string) => {
  try {
    const response = await getJobs(cid, ns) as ApiResponse<JobRow[]>;
...
const fetchJobYaml = async (cid: number, ns: string, jobName: string) => {
  try {
    const response = await getJob(cid, ns, jobName) as ApiResponse<any>;
...
const refreshClick = () => {
  ...
  fetchJobsData(clusterId.value!, namespace.value!);
};
...
const handleYamlClick = (row: JobRow) => {
  name.value = String(row.name || "");
  dialogStatus.value = "update";
  fetchJobYaml(clusterId.value!, namespace.value!, name.value);
  dialogVisible.value = true;
};
...
const handleSubmit = async (operation: "create" | "update") => {
  try {
    ...
    const response = operation === "create"
      ? await createJob(clusterId.value!, namespace.value!, jsonData) as ApiResponse
      : await updateJob(clusterId.value!, namespace.value!, name.value, jsonData) as ApiResponse;

    if (!response.success) {
      showApiError(`Job ${operation === "create" ? "创建" : "更新"}失败. 错误信息: ${response.msg}`);
      return;
    }
    dialogVisible.value = false;
    await fetchJobsData(clusterId.value!, namespace.value!);
    message(`Job ${operation === "create" ? "创建" : "更新"}成功!`, { type: "success", duration: 5000 });
  } catch (error) {
...
const handleDelete = async (jobName: string) => {
  try {
    ...
    const response = await deleteJob(clusterId.value!, namespace.value!, jobName) as ApiResponse;
    if (!response.success) {
      showApiError(`资源 '${jobName}' 删除失败. 错误信息: ${response.msg}`);
      return;
    }
    await fetchJobsData(clusterId.value!, namespace.value!);
...
<template>
  <div class="app-container">
    ...
    <el-table :data="pagedData" stripe>
      ...
      <el-table-column prop="completions" label="目标完成数" width="120" />
      <el-table-column prop="succeeded" label="成功数" width="100" />
      <el-table-column prop="failed" label="失败数" width="100" />
      <el-table-column prop="active" label="运行中" width="100" />
