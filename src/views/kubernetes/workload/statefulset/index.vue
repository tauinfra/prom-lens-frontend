...
import {
  getClusters,
  getNamespaces,
  updateStatefulSet, createStatefulSet, restartStatefulSet, scaleStatefulSet, getStatefulSet, getStatefulSets, deleteStatefulSet, batchDeleteStatefulSets
} from '@/api/kubernetes'
...
const { goToStatefulSetDetail } = useDetailRoutes();
...
const name = ref() // StatefulSet name

const data = ref<StatefulSet[]>([]);
const selectedStatefulSetNames = ref<string[]>([])
...
const filteredData = computed(() => {
  ...
  return data.value.filter((item: StatefulSet) =>
...
const handleCascaderChange = async (value: string[]) => {
  if (value && value.length === 2) {
    ...
    await fetchStatefulSets(clusterId.value, namespace.value)
  } else {
    data.value = []
  }
}
 ...
 * 获取 StatefulSet 列表
 */
const fetchStatefulSets = async (clusterId: number, ns: string) => {
  try {
    const response = await getStatefulSets(clusterId, ns) as ApiResponse<[]>
    data.value = response.data ?? []
    selectedStatefulSetNames.value = []
  } catch (error) {
    showApiError(error)
  }
}
...
/**
 * 获取 StatefulSet YAML 配置
 */
const fetchStatefulSet = async (clusterId: number, ns: any, name: string) => {
  try {
    const response = await getStatefulSet(clusterId, ns, name) as ApiResponse<[]>
    if (!response.success) {
...
const refreshClick = () => {
  ...
  fetchStatefulSets(clusterId.value, namespace.value)
}
...
const handleYamlClick = (row: any) => {
  ...
  fetchStatefulSet(clusterId.value, namespace.value, row.name);
  dialogVisible.value = true
}
...
const handleSubmit = async (operation: 'create' | 'update') => {
  try {
    ...
    const response = operation === 'create'
      ? await createStatefulSet(clusterId.value, namespace.value, jsonData) as ApiResponse
      : await updateStatefulSet(clusterId.value, namespace.value, name.value, jsonData) as ApiResponse;
    ...
    message(`StatefulSet '${name.value}' ${operation === 'create' ? '创建' : '更新'}成功！`, {
...
const handleRestart = async(name: string) => {
  try {
    ...
    const response = await restartStatefulSet(clusterId.value, namespace.value, name) as ApiResponse
...
const handleScale = async () => {
  try {
    ...
    const response = await scaleStatefulSet(clusterId.value, namespace.value, name.value, dataForm) as ApiResponse
...
const handleSelectionChange = (selection: StatefulSet[]) => {
  selectedStatefulSetNames.value = selection.map(item => item.name).filter(Boolean)
}
...
const handleBatchDelete = async () => {
  ...
  if (selectedStatefulSetNames.value.length === 0) {
    message('请先选择要删除的 StatefulSet', { type: 'warning', duration: 5000 })
    return
  }
  try {
    await ElMessageBox.confirm(
      `将删除 ${selectedStatefulSetNames.value.length} 个 StatefulSet，此操作不可撤销，确定要执行删除操作吗？`,
      '提示内容',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    const response = await batchDeleteStatefulSets(clusterId.value!, namespace.value!, { names: selectedStatefulSetNames.value }) as ApiResponse
    if (!response.success) {
      showApiError(`StatefulSet 批量删除失败. 错误信息: ${response.msg}`)
      return
    }
    await fetchStatefulSets(clusterId.value!, namespace.value!)
...
const handleDeleteConfirm = async (name: string) => {
  try {
    ...
    const response = await deleteStatefulSet(clusterId.value, namespace.value, name) as ApiResponse
    if (!response.success) {
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button
        v-show="selectedStatefulSetNames.length > 0"
        type="danger"
        plain
    ...
    <el-table :data="pagedData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="replicas" label="副本集(就绪/期望)">
        <template #default="{ row }">
          {{ row.availableReplicas || 0 }}/{{ row.replicas || 0 }}
        </template>
      </el-table-column>
      <el-table-column prop="updatedReplicas" label="更新副本集"></el-table-column>
      <el-table-column prop="availableReplicas" label="可用副本集"></el-table-column>
      <el-table-column prop="createdAt" label="创建时间"></el-table-column>
      <el-table-column fixed="right" label="操作" width="320">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="goToStatefulSetDetail(
                    {
                    id: clusterId,
                    namespace: namespace,
                    name: scope.row.name,
